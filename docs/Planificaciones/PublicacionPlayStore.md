...
- **Firma de release — hecho (2026-08-21).** El build de release usaba la clave de *debug* (la misma de cualquier proyecto Flutter nuevo, no válida para publicar). Se generó un keystore de subida real (`~/keystores/patas_al_dia/upload-keystore.jks`, alias `patasaldia_upload`, contraseñas aleatorias) y se conectó en `android/app/build.gradle.kts` vía `android/key.properties` (gitignorado, nunca se sube al repo). Es una "clave de subida" para Play App Signing, no la clave final con la que Play distribuye la app — perderla no es catastrófico (Google puede resetearla), pero igual conviene tenerla respaldada en un lugar seguro (fuera de este equipo también, por si se pierde el disco).
  ...
  - **Última generación: 2026-08-24, versionCode 5** — corrige un bug grave descubierto en producción: los `.aab` con `versionCode` 2 a 4 se quedaban en el splash (logo + pantalla negra) sin abrir nunca, confirmado por dos testers reales y por el usuario mismo reinstalando desde Play Store. Causa: una optimización de AAPT2 (no el shrinker) acorta el nombre de todos los recursos en cualquier build de release, y `flutter_local_notifications` busca su ícono por nombre en tiempo de ejecución — al no encontrarlo tira una excepción no capturada antes de `runApp()`. Ver la entrada del 2026-08-24 en `decisiones_arquitectura.md` para el diagnóstico completo. **Hay que resubir este `.aab` al track de Testing cerrado — los anteriores (versionCode 2-4) no sirven, ningún tester puede abrir la app con esos.** Cuando se agregue código nuevo antes de subir a Play Console, hay que volver a correr `flutter build appbundle --release` — el `.aab` no se actualiza solo.
