...
## Pendiente antes de poder publicar (no relacionado con privacidad)

- **`applicationId`/`namespace` todavía es `com.example.patas_al_dia`** (`android/app/build.gradle.kts`) — el placeholder por defecto de Flutter. Google Play **rechaza** publicar con un id `com.example.*`; hay que cambiarlo a algo propio (ej. `dev.breakpointx.patasaldia` o similar) antes de crear la ficha en Play Console. Cambiar esto después de publicar la primera versión ya no es posible sin perder el historial de la app (Play Store trata un cambio de `applicationId` como una app distinta) — conviene resolverlo en una sesión aparte, con tiempo, porque también puede afectar los esquemas de deep link (`patasaldia://reset-password`, ver `supabaseConfig.md`) y las Redirect URLs configuradas en el panel de Supabase.
