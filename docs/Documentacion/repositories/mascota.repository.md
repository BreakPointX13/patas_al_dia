...
- **`guardarDesdeSync`** es el método que usa el motor de sync para escribir una fila que **llegó** de Supabase (equivalente local del `upsert` remoto). **Ya no usa `db.insert(..., conflictAlgorithm: ConflictAlgorithm.replace)`** (2026-08-21, corrige un bug real de pérdida de datos — ver `decisiones_arquitectura.md`): `INSERT OR REPLACE` de SQLite, ante un conflicto de `PRIMARY KEY`, primero **borra** la fila existente antes de insertar la nueva — y con `PRAGMA foreign_keys = ON` (ver `database.helper.md`, punto 8b), ese borrado oculto disparaba el `ON DELETE CASCADE` real hacia `agenda_eventos`/`documentos`: traer por pull la edición de una mascota que ya tenía agenda o documentos locales los borraba de verdad (no soft-delete), sin que el pull los hubiera tocado para nada. Se reemplazó por un upsert manual armado a mano (`INSERT INTO mascotas (...) VALUES (...) ON CONFLICT(id) DO UPDATE SET ...`), que ante una fila existente ejecuta un `UPDATE` real — un `UPDATE` no dispara ninguna acción de foreign key, a diferencia de un `DELETE` (ni siquiera uno oculto dentro de un `REPLACE`).
- **`pendiente_push` se fija en `0` de forma explícita** dentro del mapa antes de armar el upsert (no se deja a un `DEFAULT` implícito, como en la primera versión) — necesario porque el nuevo upsert solo actualiza en el `DO UPDATE SET` las columnas que se le pasan explícitamente; sin este seteo, una fila que perdió un conflicto (gana el remoto) podría quedar con `pendiente_push = 1` colgado de antes, generando un reenvío innecesario (inofensivo pero redundante) en la próxima sincronización.
