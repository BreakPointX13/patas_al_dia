...
| `String idioma` | `TEXT DEFAULT 'sistema'` | Directo (`as String?`) | Preferencia de idioma (2026-08-18, ver `sistemaIdiomas.md`) — mismo criterio exacto que `tema`: `'sistema'` sigue el idioma del sistema operativo (si está soportado), o `'es'`/`'en'`/`'pt'` si el usuario eligió uno explícito |
| `bool avisoMapaVisto` | `INTEGER DEFAULT 0` | Evaluación lógica binaria (igual que `esInvitado`) | Marca si ya vio el aviso de política de uso del módulo Mapa (2026-08-19, ver `mapaScreen.md`) — evita mostrarlo de nuevo en cada visita a la pestaña. Estado puramente local, igual que `sesionActiva`: no se replica en la tabla `usuarios` de Supabase |
