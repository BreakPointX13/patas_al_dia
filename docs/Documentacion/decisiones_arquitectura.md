...
**De paso, tooling:** se activó `org.gradle.caching`/`org.gradle.configuration-cache` en `android/gradle.properties` para acelerar builds repetidos sin pedir más RAM (a diferencia de `org.gradle.parallel`, descartado — esta máquina ya viene con problemas recurrentes de memoria durante los builds, ver memoria de sesión `project_dispositivo_pruebas_quirks`).
