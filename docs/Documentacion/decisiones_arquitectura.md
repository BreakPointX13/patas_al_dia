...
**Un tercer punto, señalado primero como opinión de diseño — resuelto el mismo día, a pedido del usuario.** Solo `FormularioMascotaScreen` usaba `SeparadorSeccionFicha` para agrupar visualmente sus campos; `FormularioAgendaEventoScreen`/`FormularioDocumentoScreen` eran listas planas. Se sumaron separadores a los dos, con un tercer patrón de uso del widget (solo ícono genérico de Material, sin texto — ver `separadorSeccionFicha.md`, punto 6): "Archivo"/"Datos"/"Fechas" en Documento, "Evento"/"Detalles" en Agenda (este último reemplazando al `Divider` liso que separaba la "segunda mitad" del formulario).
