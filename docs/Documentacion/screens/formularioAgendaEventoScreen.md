...
### 8. Separadores de sección con `SeparadorSeccionFicha` (2026-08-22)

Mismo pedido y mismo criterio que en `FormularioDocumentoScreen` (ver `formularioDocumentoScreen.md`, punto 7, y `separadorSeccionFicha.md`, punto 6) — constructor base del widget, solo ícono, sin texto. Dos secciones, no cuatro: **Evento** (`Icons.event_note_outlined`) al principio, cubriendo mascota/título/tipo/fecha/recordatorio; y **Detalles** (`Icons.list_alt_outlined`) reemplazando al `Divider(height: 32)` que antes separaba la "segunda mitad" (observaciones, medicamentos, documentos, próxima consulta — ver punto 2, `_segundaMitadVisible`). No se agregó un separador propio para "Medicamentos" ni "Documentos adjuntos" dentro de esa segunda mitad — ya tienen su propio encabezado en negrita (`Text(..., style: titleMedium)` + botón "Agregar"), lo bastante distinto visualmente como para no necesitar también el ícono/línea de `SeparadorSeccionFicha` encima.
