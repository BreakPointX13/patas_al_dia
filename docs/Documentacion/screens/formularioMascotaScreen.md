...
### 14. `_guardando` — indicador de carga en el botón (2026-08-22)

Encontrado en una revisión de consistencia visual entre pantallas pedida por el usuario: este formulario (junto con `FormularioDocumentoScreen` y `FormularioAgendaEventoScreen`) era el único que no tenía la bandera `_guardando` que sí usan `RegistroScreen`, `CambiarContrasenaScreen` y el resto de los formularios de la app — el botón "Guardar" quedaba tocable durante todo el trabajo asíncrono de `_guardar()` (copiar la foto a un directorio persistente, escribir en la base), sin ningún aviso visual ni protección contra un doble toque. Se agregó `bool _guardando = false`, un `try/finally` alrededor del cuerpo de `_guardar()` (`setState(() => _guardando = true)` antes, reset en el `finally` solo si `mounted`), y el botón pasa a `onPressed: _guardando ? null : _guardar` con un `CircularProgressIndicator` reemplazando el texto mientras dura. Mismo patrón exacto que ya usaban los formularios de autenticación — ver `decisiones_arquitectura.md`.
