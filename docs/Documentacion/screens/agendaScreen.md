...
- **`tipoEventoMostrar(l10n, evento.tipoEvento, evento.tipoEventoPersonalizado)`**: si `tipoEvento == 'Otro'` y hay `tipoEventoPersonalizado`, muestra ese texto libre; si no, el tipo tal cual. Mismo patrón que `DocumentoModel`/`documentos_screen.dart` con `tipoDocumentoPersonalizado`. Hasta el 2026-08-22 esta pantalla tenía su propio helper privado `_tipoEventoTexto` con la misma rama duplicada en `DetalleAgendaEventoScreen` — se unificó sumando el parámetro directo a `tipoEventoMostrar` (ver `etiquetasLocalizadas.md`, punto 4, y `decisiones_arquitectura.md`).
