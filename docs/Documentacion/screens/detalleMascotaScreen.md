...
Nuevo `ListTile` entre "Documentos" y "Eliminar mascota", navega a `FormularioReporteMascotaExtraviadaScreen(mascota: mascota, tipo: 'perdido')` — ver `formularioReporteMascotaExtraviadaScreen.md`, punto 0, para las otras dos variantes del mismo formulario (accesibles desde el FAB de `MapaScreen`, no desde acá). A diferencia de "Agenda"/"Documentos" (que reciben solo `mascotaId`), acá se pasa la `MascotaModel` completa: el formulario necesita nombre/especie de la mascota para copiarlos dentro del reporte al publicar (denormalización, ver `mascotaExtraviada.model.md`), no solo un id para volver a buscarla.
