...
`tipoDocumentoMostrar` es compartida entre los documentos adjuntos a un evento de agenda (`FormularioAgendaEventoScreen`/`DetalleAgendaEventoScreen`) y, cuando se traduzca, la futura pantalla general de Documentos — mismo tipo de dato, mismo criterio de traducción, sin importar desde qué pantalla se llama.
