...
- **`applicationId` de Android: `patas_al_dia.app`** — cambiado desde el placeholder `com.example.patas_al_dia` de Flutter antes de la primera publicación (Google Play rechaza `com.example.*`). Pasó primero por `dev.breakpointx.patasaldia` (dominio invertido, la convención habitual), pero la app en Play Console ya había quedado creada con `patas_al_dia.app` como nombre de paquete — Play Console no permite cambiarlo después de creada la app, así que se adoptó ese nombre como definitivo en vez de recrear la app. **Solo afecta a Android**: iOS/macOS mantienen `dev.breakpointx.patasaldia` como bundle identifier — no hay ninguna restricción de Play Console sobre ellos, y de hecho `patas_al_dia.app` no sería válido ahí (Apple no acepta guion bajo en el Bundle ID).
